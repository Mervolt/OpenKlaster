...
export class InstallationViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const observableSummary = this.installationsService.getInstallationSummary(this.cookieService, this.installationIdOnlyNumber);
    observableSummary.subscribe(response => {
      this.installationSummary = InstallationSummary.fromDto(response);
      const outp = new Map<string, any>();
  ...
  async getInstallation(id: number) {
    const observableInstallation = this.installationsService.getInstallation(this.cookieService, id);
  ...
  stripInstallationIdIfRequired(installationId: string) {
    if (!installationId.includes(':')) {
      ...
      const splitInstallation = installationId.split(':');
      return Number(splitInstallation[1]);
    }
  }

  openDeleteConfirmationWindow() {
    const dialog = this.dialog.open(DeleteInstallationDialogComponent, {
      width: '500px'
    });
